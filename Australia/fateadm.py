...
"""
@Author : ZhaoBin
@Date   : 2018-12-11 10:04:03
@Last Modified by   : ZhaoBin
@Last Modified time : 2018-12-11 10:04:03
"""
import hashlib
import json
import time
...
class FateadmApi():
    ...
    pd_key = settings.PD_KEY
    app_id = settings.APP_ID
    app_key = settings.APP_KEY
    host = settings.FATEA_PRED_URL
    ...
    def Justice(self, request_id):
        ...
        rsp = HttpRequest(url, param)
        if rsp.ret_code == 0:
            LOG("justice succ ret: {} request_id: {} pred: {} err: {}".format(
                rsp.ret_code, rsp.request_id, rsp.pred_rsp.value, rsp.err_msg))
        else:
            LOG("justice failed ret: {} err: {}".format(
                rsp.ret_code, rsp.err_msg.encode('utf-8')))
        return rsp
...
def Captcha(img_data=None, pred_type=None, operating=1, path=None, rsp=None):
    """ 调用打码API {"rsp.pred_rsp.value": "识别结果"}
    参数 img_data: 图片数据流
    参数 pred_type: 验证码类型{"3x":"英数","1x":"数","2x": "英"} x: 位数
    参数 operating:
        1 : url 返回识别结果
        2 : 文件返回识别结果
        3 : 查询余额
    ...
    if operating == 1:
        return api.Predict(pred_type, img_data=img_data)
    elif operating == 2:
        # 通过文件形式识别：
        return api.PredictFromFile(pred_type, path)
    elif operating == 3:
        # 查询余额
        return api.QueryBalc()
    elif operating == 4 and rsp.ret_code == 0:

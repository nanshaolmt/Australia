...
"""
@Author : ZhaoBin
@Date   : 2018-11-29 11:07:00
@Last Modified by   : ZhaoBin
@Last Modified time : 2018-12-11 10:03:35
"""
...
class Base:
    ...
    # 复选 检测元素 / 点击 / 发送字符 / 选择下拉框
    def Waits(self, idName=None, timeout=3, **kwargs):
        """ 设置显性等待, 每 0.3s 检查一次
        Parameter:
            idName  : 选择器规则, 默认idName
            text    : 需要发送的信息 (非 NC --> 'noClick')
    ...
    def get_captcha(self, elem=None, pred_type=None):
        ...
        rsp = Captcha(img, pred_type=pred_type)
        return rsp
...
if __name__ == "__main__":
    ...
    print(f"{(int(bd(bd('OXY4TlhNM2JqaEtObms0V0RZaE5Tbz0=')[3:])[::2]) + 7311) / 6:0>6.0f}")

...
class Implement_class(huilian_selenium.Huilian):
    ...
    def __init__(self, id, extra="", steps="", app_id=""):
        ...
        sql, val = self.asql.select_sql(tb=self._tb_info, cond={"status": 8} if not app_id else {"id": app_id})
        # 测试
        ...
        self.Tools = huilian_selenium.Tools(**params)
    ...
    def initParmar(self):
        try:
            ...
            # 创建组时, 选择类型: 1: 家庭 2: 学校/学习 3: 奖励旅游 4: 工作/雇主 5: 朋友 6: 其他 7: 娱乐 8: 运动队/体育赛事
            ...
            self.place_issue_country = self.app["place_issue_country"]
            ...
            self.oldGoAustraliaPassport = str(self.app['australia_oldpassis'] + 1)
            # 申请人有澳大利亚签证申请号吗？
            ...
            self.address = self.app['live_address'].upper()                     # 居住街道
            ...
            self.occasion_entry = str(self.app["occasion_entry"] + 1)
            # 不知道的理由(Give reason)
            ...
            # 12 就业状况 1 在职, 2 自我雇佣, 3 失业, 4 退休, 5 学生, 99 其他
            ...
            if visa_account["payment_card_status"] is 1:
                ...
                visa_account = self.asql.getOne(*self.asql.select_sql(tb=self._tb_visa, cond={"mpid": 16}))
                self.pay_cord = visa_account["payment_card"]
                self.pay_year, self.pay_month, _ = visa_account["payment_card_time"][2:].split("-")
                self.pay_name = visa_account["payment_card_name"]
                self.pay_cvv = visa_account["payment_card_cvv"]
    ...
    def get_together(self):
        ...
        if not self.app["spouse_together"]:
        ...
        try:
            ...
            if self.Tools.is_empty(self.application_id):
                ...
                if self.app['application_status'] == 1:
                    self.log(f"填写信息")
                    self.click((By.ID, 'defaultActionPanel_0_1'), 2)
                    # self.click((By.ID, 'actionPanel_0_0'), 2)
                    # sleep(1)
                    # self.page1()
                    return 1
        ...
        try:
            ...
            if nums() != len(self.fileList):
                ...
                fileinfo = {
                    ...
                    "retirement_certificate"        : "Retirement certificate (retirees)",      # 退休证(退休人员)      - DT417
                ...
                """
                    ...
                    财务状况和访问资金的证据 - 资金证明
                        ["proof_funds", "sponsor_income_certificate", "land_title"]  # , "spouse_parental_funding", "auxiliary_assets"
                    目前就业或自营职业的证据
                ...
                recommendes = {
                    ...
                    "Evidence of the financial status and funding for visit":
                        ["proof_funds", "land_title", "spouse_parental_funding", "sponsor_income_certificate"],
